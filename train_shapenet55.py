...
from utils.data_loaders import ShapeNet
from easydict import EasyDict as edict
from importlib import import_module
from pprint import pprint
from manager import Manager, Manager_shapenet55
from models.pointmac import CRAPCN_sn55, CRAPCN_sn55_d
from models.pointmac import ExtendedModel, ExtendedModel2
...
def train_net(cfg):
    ...
    ########################
    ##ShapeNet
    train_dataset_loader = ShapeNet('train')
    val_dataset_loader = ShapeNet('test')
    ...
    val_data_loader = torch.utils.data.DataLoader(val_dataset_loader,
                                                  batch_size=cfg.TRAIN.BATCH_SIZE,
    ...
    extend1 = ExtendedModel(model)
    extend2 = ExtendedModel2(model, extend1)
    if torch.cuda.is_available():
        model = torch.nn.DataParallel(model).cuda()
        extend1 = torch.nn.DataParallel(extend1).cuda()


    ##################
    # Training Manager
    ##################
    manager = Manager_shapenet55(model, extend1, cfg, extend2=extend2)

    # Start training
    manager.train(model,extend1, train_data_loader, val_data_loader, cfg)
